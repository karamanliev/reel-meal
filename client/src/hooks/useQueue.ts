...
export function useQueue() {
  ...
  const [inputText, setInputTextState] = useState(initialText); const [sourceImages, setSourceImagesState] = useState<File[]>([]); const [customImage, setCustomImageState] = useState<File | null>(null); const [customImageUrl, setCustomImageUrlState] = useState("");
  ...
  const setCustomImageUrl = useCallback((value: string) => { formRevisionRef.current++; setCustomImageUrlState(value); }, []);
  ...
  const addJob = useCallback(async () => {
    ...
    for (const image of sourceImages) form.append("sourceImages", image); if (useCustomImage && customImage) form.set("customImage", customImage); else if (useCustomImage && customImageUrl.trim()) form.set("customImageUrl", customImageUrl.trim());
    try { const response = await fetch("/api/parse", { method: "POST", body: form }); const data = await response.json().catch(() => ({})) as { error?: string }; if (!response.ok) throw new Error(data.error || "Submission failed."); if (formRevisionRef.current === submittedRevision) { setInputTextState(""); setSourceImagesState([]); setCustomImageState(null); setCustomImageUrlState(""); setUseCustomImage(false); } }
  ...
  }, [inputText, sourceImages, customImage, customImageUrl, extractTranscript, autoImport, useCustomPrompt, customPrompt, useCustomImage, updateJob]);
