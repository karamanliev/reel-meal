...
- always requests a second AI estimate for calories, protein, carbohydrates, and fat per 100 g of the finished cooked dish
...
Every generated recipe includes an AI nutrition estimate for 100 g of the estimated finished cooked dish. Explicit nutrition from the source is preferred when its basis is clear; otherwise ReelMeal estimates values from the normalized ingredients, instructions, and expected cooked yield.

Mealie stores these values without a per-100 g basis. With the current SparkyFitness Mealie integration, treat `1 serving` as `100 g` for recipes generated by this feature. For example, `1.5 servings` represents `150 g`.

These values are approximations. Ingredient ambiguity, brands, substitutions, cooking loss, water absorption or evaporation, and discarded ingredients can all affect the result. If estimation fails after retrying, ReelMeal still imports the recipe without nutrition and displays a nonfatal warning.
